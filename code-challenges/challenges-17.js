...
const filterLinks = (str) => {
  let pattrn = /^(http)+[a-zA-Z:/.]+(.com|.org|.net)$/;
  return str
    .split(`"`)
    .filter((e) => pattrn.test(e))
    .join("")
    .slice(7);
  // write your code here
};
...
const isPalindrome = (str) => {
  ...
  let text = [...arr].join("");
  let reversedText = arr.reverse().join("");
...
const samePattern = (str, arr) => {
  ...
  let arrayFromStrReversed = str.split("").reverse();

  if (
    JSON.stringify(array) === JSON.stringify(reversedArray) &&
    JSON.stringify(arrayFromStr) === JSON.stringify(arrayFromStrReversed)
  ) {
    return true;
  } else if (
    JSON.stringify(array) !== JSON.stringify(reversedArray) &&
