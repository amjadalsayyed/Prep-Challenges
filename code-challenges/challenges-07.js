...
const objLat = (obj) => {
  ...
  return `my name is ${firstNameCap} ${lastNameCap} I am ${obj.age} YO, and I love ${obj.hobby}.`;
};
// -------------------------------------------------------------------------------------------------------
...
const cvFormatter = (arr) => {
  let arr2 = arr.filter((i) => i.yearsOfExperience > 1);
  let arr3 = arr2.map((e) => {
    e = {
...
const classesAvg = (data) => {
  data.grades.forEach((element) => {
    element.classes.map((singleclass) => {
      singleclass.avg = Math.floor(
        singleclass.classScores.reduce((acc, current) => acc + current, 0) /
          singleclass.classScores.length
      );
    });
  });
  return data;
